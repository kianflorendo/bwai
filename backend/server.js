...
if (supabaseUrl && supabaseKey) {
  ...
  console.log("Supabase client initialized in backend.");
} else {
  console.warn("Warning: Missing SUPABASE_URL or SUPABASE_ANON_KEY in environment. Supabase endpoints will fail.");
}

// Allow CORS for the new endpoints if called directly
...
app.post('/api/commands', async (req, res) => {
  if (!supabase) return res.status(500).json({ error: "Supabase not configured on backend." });
  const { command_code, device_target } = req.body;
  
  if (!command_code) return res.status(400).json({ error: "command_code is required" });

  const { data, error } = await supabase
    .from('device_commands')
    .insert([{ command_code, device_target: device_target || 'led_indicator_array', executed: false }]);
